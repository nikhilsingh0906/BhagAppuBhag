var appu, speed=5, platform, f=0.3, jumpCount=0, end=0;
...
if(appu.collide(knife1) || appu.collide(knife2)) {
  appu.remove();
  knife1.remove();
  end = 1;
}

if(appu.position.x < 5) {
  appu.position.x = 5;
}

if(appu.position.x > width - 10) {
  end = 2;
}
drawSprites();
drawSprites(sun);
